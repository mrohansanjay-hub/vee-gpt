...
export default function Sidebar({ isOpen, onClose, isLoggedIn, name, email, onLogin, chatHistory, onNewChat, onLoadChat, onDeleteChat, onUpdateChat }) {
      ...
      {renameId === chat.session_id ? (
        <div className="flex items-center gap-2 w-full" onClick={e => e.stopPropagation()}>
          <input 
            ...
            className="bg-gray-700 text-white text-lg rounded px-2 py-1 w-full outline-none border border-indigo-500"
            autoFocus
            onKeyDown={(e) => {
        ...
        <>
          <button onClick={() => onLoadChat(chat.session_id)} className="flex items-center gap-3 text-left text-lg text-gray-400 hover:text-white truncate flex-1 min-w-0">
            ...
            {menuOpenId === chat.session_id && (
              <div className="absolute right-0 top-6 w-32 bg-gray-800 border border-gray-700 rounded shadow-xl z-50 overflow-hidden flex flex-col py-1" onClick={e => e.stopPropagation()}>
                <button 
                  ...
                  className="flex items-center gap-2 px-3 py-2 text-base text-gray-300 hover:bg-gray-700 hover:text-white w-full text-left"
                >
                ...
                <button 
                  ...
                  className="flex items-center gap-2 px-3 py-2 text-base text-red-400 hover:bg-gray-700 hover:text-red-300 w-full text-left"
                >
  ...
  return (
    <>
      ...
      >
        ...
        <div className="p-4 flex items-center justify-between border-b border-gray-800 h-14">
          <div className="flex items-center gap-3">
             ...
             <h1 className="text-2xl font-bold tracking-wide">Vee-GPT</h1>
          </div>
          ...
          {isSearching ? (
            <div className="flex items-center gap-2 p-3 bg-gray-800 rounded-lg border border-indigo-500/50">
              ...
              <input 
                ...
                autoFocus
                placeholder="Search..."
                className="bg-transparent border-none outline-none text-white text-lg w-full placeholder-gray-500 min-w-0"
                value={searchQuery}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 border-t border-gray-800">
          <h3 className="text-sm font-semibold text-gray-500 uppercase mb-3 tracking-wider">Vee Chat</h3>
          <div className="flex flex-col gap-1">
            {/* Dynamic Chat History */}
            {displayChats.length > 0 ? (
              displayChats.map((chat) => <ChatItem key={chat.session_id} chat={chat} />)
            ) : (
              <div className="text-gray-500 text-base p-2 italic">No chats found</div>
            )}
          </div>
        </div>
      ...
      {/* Settings Modal */}
      {showSettings && (
        <div className="settings-modal-overlay" onClick={() => setShowSettings(false)} style={{marginBottom:"100px"}}>
          <div className="settings-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <h3 className="text-sm font-semibold text-gray-500 uppercase mb-3 tracking-wider">Theme</h3>
            ...
            {isLoggedIn && pinnedChats.length > 0 && (
              <div className="mb-4 border-t border-gray-700 pt-4">
                <h3 className="text-sm font-semibold text-gray-500 uppercase mb-3 tracking-wider flex items-center gap-2">
            ...
            {/* Account Section */}
            <div className="mb-4 border-t border-gray-700 pt-4">
              <h3 className="text-sm font-semibold text-gray-500 uppercase mb-3 tracking-wider">Account</h3>
              {isLoggedIn ? (
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-3 p-2 bg-gray-800 rounded-lg">
                    <FaUser className="text-indigo-400" />
                    <div className="flex flex-col overflow-hidden">
                      <span className="text-base font-medium text-white truncate">{name || "User"}</span>
                      <span className="text-sm text-gray-400 truncate" title="Email is encrypted for privacy">{maskEmail(email)}</span>
                    </div>
                  </div>
